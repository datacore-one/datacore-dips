...
| Personal draft about work? | Personal: `1-active/` - not official yet |
...
├── journal/                   # Daily journals (Obsidian)
│   └── YYYY-MM-DD.md
├── 0-inbox/                   # Unprocessed captures
├── 1-active/                  # Focus areas (self-contained work)
│   ├── trading/               # Trading work
│   ├── health-longevity/      # Health & wellness
│   ├── fds/                   # FDS work
│   └── [focus-area]/          # Other focus areas
├── 2-code/                    # Personal projects
├── 3-knowledge/               # Permanent knowledge (cross-cutting)
│   ├── zettel/                # Atomic concepts
│   ├── literature/            # Source summaries
│   ├── reference/             # People, companies, entities
│   │   ├── people/            # Contact profiles
│   │   └── companies/         # Company profiles
│   ├── pages/                 # Wiki pages
│   └── clippings/             # Web clippings
├── 4-outbox/                  # Content EXIT (last numbered folder)
...
For folders with many non-AI-readable files (e.g., telegram exports, video collections), creating individual companions is impractical. Instead, use a **folder-level media index**.

**Format**: `_media-index.md` in the folder containing media files.

```markdown
---
type: media-index
folder: telegram-export/chat_0068/files
...
# Media Index: chat_0068/files

Non-AI-readable files in this folder.

## Audio (23 files, 45MB)

| File | Size | Date | Description |
|------|------|------|-------------|
| prenos.wav | 2.1MB | 2025-12-21 | Voice message |
| audio-2.wav | 1.8MB | 2025-12-21 | Voice message |

## Video (12 files, 78MB)

| File | Size | Date | Description |
|------|------|------|-------------|
| clip-2024-03-15.mp4 | 15MB | 2024-03-15 | [needs description] |

---

*Generated by structural-integrity. Edit descriptions manually or run `/ingest` for AI summaries.*
```

**When to use Media Index vs Individual Companions:**

| Pattern | Use When | Behavior |
|---------|----------|----------|
| **Individual Companion** | Single important file needing detailed documentation | 1:1 mapping: `video.mp4` → `video.md` |
| **Media Index** | Bulk media, exports, archives | 1:many mapping: `_media-index.md` covers folder |

**Media categories** (grouped in index):
...
**Structural Integrity modes:**

| Mode | Flag | Behavior |
|------|------|----------|
| **Index mode** (default) | - | Reports one warning per folder missing `_media-index.md` |
| **Strict mode** | `--strict` | Reports one warning per file missing individual companion |

**Auto-generation:**

```bash
# Generate indexes for folders with media files
/structural-integrity fix

# Preview what would be generated
/structural-integrity fix --dry-run
```

**Reserved filename**: `_media-index.md` is a system file; do not use for other purposes.
...
| `my-thoughts-on-fundraising.md` | 0-personal/ | Personal perspective |
| `pitch-deck-v3.key` | 1-datafund/1-tracks/comms/ | Official company asset |
| `pitch-ideas-draft.md` | 0-personal/1-active/ | Personal draft, not official |
...
| Personal writings | Personal | `1-active/` or `3-knowledge/pages/` | Personal processing |
| Historical/outdated | Org | `4-outbox/archive/` | Companion only, routed to archive repo |
| Personal research | Personal | `3-knowledge/` | Extract personal zettels |
...
| **Missing media index** | Folder with media files but no `_media-index.md` (default mode) | Warning |
| **Missing companions** | `.key` file without `.md` companion (strict mode) | Warning |
